function xsect_delta = calc_xsect_delta(hv, element, corelevel, formalism, extrapolate, plot_results)
% xsect_delta = calc_xsect_delta(hv, element, corelevel, formalism, extrapolate, plot_results)
...
formalism_yl1985     = [...
    ...
    "YL(1985)", "(1985)YL", "YL1985", "1985YL", "Y1985", "1985Y"...
...
if ~isempty(find(strcmpi(formalism_s1973, formalism),1))
    xsect_delta = NaN(length(hv), length(corelevel));
    msg = 'The dipole asymmetry parameter delta is not available in the Scofield1973 database. Delta set to NaN.'; 
    disp(msg);
% -- YehLindau1985 formalism
elseif ~isempty(find(strcmpi(formalism_yl1985, formalism),1))
    xsect_delta = NaN(length(hv), length(corelevel));
    msg = 'The dipole asymmetry parameter delta is not available in the Yeh1985 database. Delta set to NaN.'; 
    disp(msg);
% -- Trzhaskovskaya2018 formalism
elseif ~isempty(find(strcmpi(formalism_t2018, formalism),1))
    xsect_delta = calc_xsect_delta_trzh2018(hv, element, corelevel, extrapolate, plot_results);
% -- Cant2020 formalism
