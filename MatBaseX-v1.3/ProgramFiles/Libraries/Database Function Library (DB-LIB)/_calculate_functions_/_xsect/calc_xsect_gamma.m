function xsect_gamma = calc_xsect_gamma(hv, element, corelevel, formalism, extrapolate, plot_results)
% sect_gamma = calc_xsect_gamma(hv, element, corelevel, formalism, extrapolate, plot_results)
...
if ~isempty(find(strcmpi(formalism_s1973, formalism),1))
    xsect_gamma = NaN(length(hv), length(corelevel));
    msg = 'The dipole asymmetry parameter gamma is not available in the Scofield1973 database. Gamma set to NaN.'; 
    disp(msg);
% -- YehLindau1985 formalism
elseif ~isempty(find(strcmpi(formalism_yl1985, formalism),1))
    xsect_gamma = NaN(length(hv), length(corelevel));
    msg = 'The dipole asymmetry parameter gamma is not available in the Yeh1985 database. Gamma set to NaN.'; 
    disp(msg);
