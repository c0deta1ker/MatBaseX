function fig = plt_result_xsect_delta(element, formalism, hv, corelevel, hv_data, xsect_data, xsect_delta)
% fig = plt_result_xsect_delta(element, formalism, hv, corelevel, hv_data, xsect_data, xsect_delta)
%   Plot phase-shift parameter (delta) vs. photon energy.
%   Linear plot of phase-shift parameter delta (typically 0–0.55) vs. photon
%   energy. Useful for understanding quantum properties of photoionization.
%
%   IN:
...
%   -   xsect_data:   (double, matrix) Delta parameter reference data
%   -   xsect_delta:  (double, scalar or vector) Delta value(s) at hv
%
%   OUT:
...
fig = figure('Name', sprintf('Photoionization Parameters (XSECT-DELTA) - %s (Z=%.0f)', element, atomicNumber));
...
for i = 1:nTransitions
    ...
    if isscalar(hv);        scatter(hv, xsect_delta(i), 'SizeData', 25, 'markeredgecolor', colorList{i}, 'markerfacecolor', colorList{i}, 'MarkerFaceAlpha', 0.95, 'HandleVisibility','off');
    elseif isrow(hv);       scatter(hv, xsect_delta(i,:), 'SizeData', 25, 'markeredgecolor', colorList{i}, 'markerfacecolor', colorList{i}, 'MarkerFaceAlpha', 0.95, 'HandleVisibility','off');
    elseif iscolumn(hv);    scatter(hv, xsect_delta(:,i), 'SizeData', 25, 'markeredgecolor', colorList{i}, 'markerfacecolor', colorList{i}, 'MarkerFaceAlpha', 0.95, 'HandleVisibility','off');
    end
end
...
ylabel('\delta', 'FontSize', 11, 'FontWeight', 'bold', 'FontName', 'Helvetica');
