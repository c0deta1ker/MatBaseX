function xsect_beta = calc_xsect_beta_yehlind1985(hv, element, corelevel, extrapolate, plot_results)
% xsect_beta = calc_xsect_beta_yehlind1985(element, corelevel, hv, plot_results)
...
xsect_beta = NaN(size(ihv, 1), length(cl_indx));
for i = 1:length(cols_with_nan)
    if cols_with_nan(i) == 1;       xsect_beta(:,i) = NaN(size(ihv));
    else
        if extrapolate == 1;        xsect_beta(:,i) = interp1(hv_data, xsect_data(:,i), ihv, 'pchip', 'extrap');
        elseif extrapolate == 0;    xsect_beta(:,i) = interp1(hv_data, xsect_data(:,i), ihv, 'pchip', NaN);
        end
    end
end
...
if plot_results == 1
    ...
    formalism       = formalismList{3}; 
    plt_result_xsect_beta(element, formalism, hv, cls, hv_data, xsect_data, xsect_beta);
end
end