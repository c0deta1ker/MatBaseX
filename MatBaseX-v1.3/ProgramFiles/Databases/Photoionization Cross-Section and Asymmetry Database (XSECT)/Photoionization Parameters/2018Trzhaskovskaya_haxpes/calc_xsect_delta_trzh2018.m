function xsect_delta = calc_xsect_delta_trzh2018(hv, element, corelevel, extrapolate, plot_results)
...
xsect_delta = NaN(size(ihv, 1), length(cl_indx));
for i = 1:length(cols_with_nan)
    if cols_with_nan(i) == 1;       xsect_delta(:,i) = NaN(size(ihv));
    else
        if extrapolate == 1;        xsect_delta(:,i) = interp1(hv_data, xsect_data(:,i), ihv, 'pchip', 'extrap');
        elseif extrapolate == 0;    xsect_delta(:,i) = interp1(hv_data, xsect_data(:,i), ihv, 'pchip', NaN);
        end
    end
end
...
if plot_results == 1
    ...
    plt_result_xsect_delta(element, formalism, hv, cls, hv_data, xsect_data, xsect_delta);
end
end